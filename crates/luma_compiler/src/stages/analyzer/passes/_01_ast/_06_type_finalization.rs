use crate::{Type, ast::*, stages::analyzer::{AnalyzerContext, AnalyzerPass, passes::_01_ast::TypeInference}};
        ...
        match &mut stmt.item {
            ...
            StmtKind::Var(var_decl) => {
                let symbol_id = var_decl.symbol.unwrap_id();
                    ...
                    Self::finalize_expr(ctx, &mut var_decl.initializer);
            },
        }
    }
}
...
impl TypeFinalization {
    fn finalize_expr(ctx: &AnalyzerContext, expr: &mut Expr) {
        let ty = match &mut expr.item {
            ExprKind::Assign(assign_expr) => todo!(),
            ExprKind::Binary(binary_expr) => {
                Self::finalize_expr(ctx, &mut binary_expr.left);
                Self::finalize_expr(ctx, &mut binary_expr.right);
            ...
            ExprKind::Literal(literal_expr) => {
                TypeInference::infer_literal_type(None, literal_expr).unwrap()
            },
